...
class ClinicalSentenceTokenizer(object):
    def __init__(self, default_sentence_tokenizer, preprocess_split_re_strs = [r'[\r\n]{2,}']):
        self.sentence_tokenizer = default_sentence_tokenizer
        
        print('Class type initialized for ClinicalSentenceTokenizer for sentence tokenization : {}'.format(type(self.sentence_tokenizer)))
    ...
    def tokenize(self, text):
        ...
        for i in range(len(start_index_sorted)):
            ...
            # Figure out which flavor of sentence breaking we are using
            sentences = None
            if isinstance(self.sentence_tokenizer, PunktSentenceTokenizer):
                sentences = self.sentence_tokenizer.tokenize(preprocessed_sentence)
            elif isinstance(self.sentence_tokenizer, RuSH):
                sentences = []
                # get sentence spans first
                sentence_spans = self.sentence_tokenizer.segToSentenceSpans(preprocessed_sentence)
                # then get the strings
                for sentence_span in sentence_spans:
                    # pull this from the string processed above
                    sentence = preprocessed_sentence[sentence_span.begin : sentence_span.end]
                    sentences.append(sentence)
            else:
                raise ValueError('Class type not currently supported for sentence tokenization : {}'.format(type(self.sentence_tokenizer)))
...
class IndexTokenizer(object):
    def __init__(self, sentence_tokenizer, word_tokenizer, keep_token_strings = False):
        ...
        print('Class type initialized for IndexTokenizer for sentence tokenization: {}'.format(type(self.sentence_tokenizer)))
